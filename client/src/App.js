...
function App() {
  ...
  if (typeof instance == "undefined") return <h1>accounts not working</h1>;
  if (typeof accounts == "undefined") {
    return <h1>accounts not working</h1>;
  } else {
    return (
      <div>
        <h1>showing Balance of smart contract</h1>
        <Balance Prop1={instance._address} prop2={balance}></Balance>
        <Sendeth prop1={sendEther} />
        {temp >= 2 ? (
          <LuckyDraw prop1={luckydraw}></LuckyDraw>
        ) : (
          <h3>More fund Needed to select a Winner......</h3>
        )}
        {console.log(count)}
      </div>
    );
